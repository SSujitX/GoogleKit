...
def test_default_token_path_uses_platform_env(
    ...
    path = default_token_path("googlekit")
    assert path == tmp_path / "googlekit" / "token.json"
    assert path.parent.is_dir()


def test_default_token_path_mkdir_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    if os.name == "nt":
        monkeypatch.setenv("APPDATA", str(tmp_path))
    else:
        monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
